...
<div class="page-head wrap">
  ...
  <p>Mỗi trò chơi mô phỏng một tình huống giao thông thực tế mà học sinh gặp hằng ngày — AI luôn đồng hành hướng dẫn, giải thích và động viên em từng bước.</p>

  <?php if (!$isStudent): ?>
  <div class="cond-row" style="margin-top:20px;">
    <div class="cond-chip" style="color:var(--yellow); border-color:rgba(251,191,36,0.4);">
      🔒 <?= $user ? 'Đăng nhập bằng tài khoản Học sinh' : 'Đăng nhập' ?> để lưu XP/Coin/Huy hiệu thật và mở khoá đầy đủ 5 trò chơi
    </div>
  </div>
  <?php endif; ?>
...
<section class="games">
  <div class="wrap">
    <div class="game-grid">
      ...
      ?>
      <div class="game-card<?= $i === 0 ? ' featured' : '' ?><?= $unlocked ? '' : ' locked' ?>" <?= $unlocked ? "onclick=\"location.href='{$g['page']}'\"" : '' ?>>
        <?php if ($i === 0): ?>
        ...
        <?php else: ?>
          <div class="game-visual"><?= $g['icon'] ?><span class="game-badge"><?= $unlocked ? '🎮 Chơi ngay' : '🔒 Khoá' ?></span></div>
          <div class="game-info">
            <h4><?= e($g['title']) ?></h4>
            <p><?= e($g['desc']) ?></p>
            <div class="reward-row"><div class="reward-chip">⭐ +<?= $g['xp'] ?> XP</div><div class="reward-chip">🪙 +<?= $g['coin'] ?> Coin</div></div>
            <?php if (!$unlocked): ?>
            <div class="lock-tag">🔒 Mở khoá ở cấp độ <?= $needLevel ?> (bạn đang ở cấp <?= $currentLevel ?>)</div>
            <?php endif; ?>
          </div>
        <?php endif; ?>
      </div>
