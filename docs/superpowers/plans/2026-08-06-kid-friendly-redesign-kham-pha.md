...
- [ ] **Step 6: Thêm section vào 2 dashboard qua partial chung** — tạo `partials/review-moderation-section.php` chứa toàn bộ markup + JS của hàng chờ duyệt (partial tự `require_once __DIR__ . '/../lib/moderation.php'` và gọi `pending_reviews($db)` — biến `$db` do dashboard cung cấp sẵn): render mỗi review 1 `.kid-card`: place_name + author + sao + content (`e()`) + ảnh thumbnail (decode `photos` JSON, `<img src="uploads/reviews/..." width="90">`) + 2 nút `.kid-btn--green` Duyệt / `.kid-btn--red` Từ chối (prompt lý do); JS inline trong partial: POST `review-moderate.php` với CSRF header → xoá card khỏi DOM khi success. Trong mỗi dashboard (`dashboard-giao-vien.php`, `dashboard-admin.php`): sidebar thêm `<a class="side-link" href="#duyet-review"><span class="ic">📝</span> Duyệt review</a>` (kèm badge số pending), cuối main thêm `<section id="duyet-review"><?php require __DIR__ . '/partials/review-moderation-section.php'; ?></section>`, và thêm meta csrf vào head nếu chưa có.
