...
document.addEventListener('DOMContentLoaded', () => {
  const missions = [
    ...
    { text: 'Chơi trò chơi "Kéo thả biển báo"', reward: '+30 XP · 50 Coin', done: false },
  ...
  function renderMissions() {
    ...
    missions.forEach(m => {
      ...
      item.onclick = () => { m.done = !m.done; renderMissions(); updateCount(); };
      list.appendChild(item);
    });
  }
  function updateCount() {
    const done = missions.filter(m => m.done).length;
    document.getElementById('missionCount').textContent = `${done}/${missions.length} hoàn thành`;
  }
  renderMissions();
  updateCount();
});
